                ...
                <div class="row">
                    <div class="col-md-12 mb-5">
                        <div class="form-group" style="float: right;">
                            @php
                                $years=App\Models\Remedies::select('year')->distinct()->orderBy('year' ,'DESC')->get();
                            @endphp
                            <form action="{{route('remedies.filter')}}" method="get" >
                        ...
                        <tbody>
                            @if ($remedies->count() > 0)
                                @foreach ($remedies as $item)
                                    <tr class="text-center">
                                        <td>{{$item->title}}</td>
                                        <td><a href="{{route('remedies.download',$item->id)}}" id="counter" data-table="remedies" data-id="{{$item->id}}" data-counter="1"><i class=" fas fa-download"></i></a></td>
                                    </tr>
                                @endforeach
                            @else
                            <tr class="text-center">
