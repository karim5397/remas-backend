...
<div class="content">
    ...
    <div class="top-bar">
        ...
        <nav aria-label="breadcrumb" class="-intro-x mr-auto hidden sm:flex">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">Show General Assembly Decision</li>
            </ol>
        </nav>
    ...
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">
            Show General Assembly Decision
        </h2>
     ...
     <div class="grid grid-cols-12 gap-6 mt-5">
         <div class="overflow-x-auto mt-4 col-span-12 sm:col-span-6 xl:col-span-8">
                 ...
                 <tbody>
                     @if (count($decisions) > 0)
                         @foreach ($decisions as $decision)
                             <tr>
                                 <td>{{$decisions->firstItem()+$loop->index}})</td>
                                 <td>{{$decision->title}}</td>
                                 <td>{{$decision->year}}</td>
                                 <td>{{$decision->file}}</td>
                                 <td>
                                     <div class="flex items-center">
                                         <form action="{{route('decision.destroy' , $decision->id)}}" method="POST" class="ml-2">
                                             @csrf
                                             @method('DELETE')
                                             <a href="" data-toggle="tooltip" title="{{__('Delete')}}" data-id="{{$decision->id}}" data-placement='button' class="dltBtn btn btn-danger"><i class="fa fa-trash"></i></a>
                                         </form>
                                     </div>
                                 </td>
                             </tr>
             ...
             <div class="mt-3 p-3">
                 ...
                 {{$decisions->links('pagination::custom')}}
