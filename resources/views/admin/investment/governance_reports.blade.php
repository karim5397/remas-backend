    ...
    <div class="top-bar">
        ...
        <nav aria-label="breadcrumb" class="-intro-x mr-auto hidden sm:flex">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">Show Governance Reports</li>
            </ol>
        </nav>
     ...
     <div class="grid grid-cols-12 gap-6 mt-5">
         <div class="overflow-x-auto mt-4 col-span-12 sm:col-span-6 xl:col-span-8">
                 ...
                 <tbody>
                     @if (count($governments) > 0)
                         @foreach ($governments as $government)
                             <tr>
                                 <td>{{$governments->firstItem()+$loop->index}})</td>
                                 <td>{{$government->title}}</td>
                                 <td>{{$government->year}}</td>
                                 <td>{{$government->count}}</td>
                                 <td>
                                     <div class="flex items-center">
                                         <form action="{{route('government.destroy' , $government->id)}}" method="POST" class="ml-2">
                                             @csrf
                                             @method('DELETE')
                                             <a href="" data-toggle="tooltip" title="{{__('Delete')}}" data-id="{{$government->id}}" data-placement='button' class="dltBtn btn btn-danger"><i class="fa fa-trash"></i></a>
                                         </form>
                                     </div>
                                 </td>
                             </tr>
             ...
             <div class="mt-3 p-3">
                 ...
                 {{$governments->links('pagination::custom')}}
