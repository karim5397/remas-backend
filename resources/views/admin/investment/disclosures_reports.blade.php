    ...
    <div class="top-bar">
        ...
        <nav aria-label="breadcrumb" class="-intro-x mr-auto hidden sm:flex">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">Show Disclosures Reports</li>
            </ol>
        </nav>
     ...
     <div class="grid grid-cols-12 gap-6 mt-5">
         <div class="overflow-x-auto mt-4 col-span-12 sm:col-span-6 xl:col-span-8">
                 ...
                 <tbody>
                     @if (count($disclosures) > 0)
                         @foreach ($disclosures as $disclosure)
                             <tr>
                                 <td>{{$disclosures->firstItem()+$loop->index}})</td>
                                 <td>{{$disclosure->title}}</td>
                                 <td>{{$disclosure->year}}</td>
                                 <td>{{$disclosure->count}}</td>
                                 <td>
                                     <div class="flex items-center">
                                         <form action="{{route('disclosure.destroy' , $disclosure->id)}}" method="POST" class="ml-2">
                                             @csrf
                                             @method('DELETE')
                                             <a href="" data-toggle="tooltip" title="{{__('Delete')}}" data-id="{{$disclosure->id}}" data-placement='button' class="dltBtn btn btn-danger"><i class="fa fa-trash"></i></a>
                                         </form>
                                     </div>
                                 </td>
                             </tr>
             ...
             <div class="mt-3 p-3">
                 ...
                 {{$disclosures->links('pagination::custom')}}
                    ...
                    <div class="intro-y box">
                        <div  class="p-5" style="border-radius:10px; border: 1px solid #000">
                            <div class="preview">
                                ...
                                <form action="{{route('disclosure.store')}}" method="POST" enctype="multipart/form-data">
                                    @csrf
